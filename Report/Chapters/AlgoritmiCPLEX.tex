\chapter{Risoluzione del problema tramite CPLEX}
...
$\mathtt{percentage}$= percentuale di fissagio degli archi\;
}
\KwOut {$\mathtt{x}$= soluzione intera senza subtour}
\BlankLine
$\mathtt{n}=0$\;
...
\BlankLine
$setTimeLimit(deadline/num\_iterations)$\;
$\mathtt{x} \gets solve(model)$\;
\BlankLine
\For{$\mathtt{j} \gets \;0,...,num\_nodi-1$}{
$k \gets random(0,1)$\;
\BlankLine
